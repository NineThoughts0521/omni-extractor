...
class HTMLCleaner:
    ...
    def _extract_visible_text(self, soup: BeautifulSoup, title: Optional[str] = None) -> str:
        ...
        def is_visible_element(element):
            ...
            if element.parent.name in ['style', 'script', 'noscript', 'iframe', 'svg', 'title']:
                return False
        ...
        visible_texts = [text.strip() for text in texts if is_visible_element(text) and text.strip()]
    ...
    def _apply_char_budget(self, text: str) -> str:
        ...
        if last_sentence_end > self.char_budget * 0.8:  # Only if it's reasonably close to budget
...
def clean_html_content(html_content: str, char_budget: int = 20000) -> Tuple[str, Optional[str]]:
